...
![synchronizedå³é®å­åç](java并发2.assets/synchronized关键字原理.png)
...
![synchronized关键字原理](java并发2.assets/synchronized关键字原理2.png)
...
![æ°æ®ä¸ä¸è´](java并发2.assets/数据不一致.png)

要解决这个问题，就需要把变量声明为**volatile**，这就指示 JVM，这个变量是不稳定的，每次使用它都到主存中进行读取。

说白了， **volatile** 关键字的主要作用就是保证变量的可见性然后还有一个作用是防止指令重排序。

![volatileå³é®å­çå¯è§æ§](java并发2.assets/volatile关键字的可见性-1587629740379.png)
